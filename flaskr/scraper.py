...
@bp.route('/process_user_list', methods=['POST'])
def analyzeUserList():
    ...
    #inputLink = request.form['userList']
...
@bp.route('/process_public_list', methods=['POST'])
def analyzePublicList():
    ...
    #inputLink = request.form['publicList']
    ...
    for row in rows:
        ...
        driver2.close()
    driver.close()
    sorted_scores = dict(sorted(bookScores.items(), key=lambda item: item[1], reverse=True))
    ...
    for rec in sorted_scores:
        print(rec, "with a score of", sorted_scores[rec])
    #print(bookScores)
